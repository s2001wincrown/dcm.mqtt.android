...
public class Connection {
    ...
    //WebApi URI for data post
    private String webApiURI = null;
     ...
     */
    public static Connection createConnection(String clientHandle, String clientId, String host, int port, Context context, boolean tlsConnection, String webApiURI){
        String uri;
        if (host.startsWith("ssl://") || host.startsWith("tcp://") || host.startsWith("ws://")){
            uri = host + ":" + port;
        }else {
            if (tlsConnection) {
                uri = "ssl://" + host + ":" + port;
        ...
        return new Connection(clientHandle, clientId, host, port, context, client, tlsConnection, webApiURI);
    }

    public void updateConnection(String clientId, String host, int port, boolean tlsConnection, String webApiURI){
        String uri;
        if (host.startsWith("ssl://") || host.startsWith("tcp://") || host.startsWith("ws://")){
            uri = host + ":" + port;
        }else {
    ...
    private Connection(String clientHandle, String clientId, String host,
                       int port, Context context, MqttAndroidClient client, boolean tlsConnection, String webApiURI) {
    ...
    /**
     * Get the webapi URI that this connection object data post to
     * @return the webapi URI that this connection object data post to
     */
    ...
    public void messageArrived(String topic, MqttMessage message){
        ...
        String webApiURI = this.getWebApiURI();
        if (webApiURI != null && !webApiURI.isEmpty()) {
            JSONObject post_dict = new JSONObject();
            try {
                post_dict.put("ExternalHtmlSource", new String(message.getPayload()));
                post_dict.put("content", topic);
                post_dict.put("ExternalType", "Other");

                Log.i("post_dict", String.valueOf(post_dict));
            ...
            if (post_dict.length() > 0) {
                try {
                    new WebApiAsyncTask().execute(String.valueOf(post_dict), webApiURI); //"http://192.168.0.180:8080/dcm-web/services/api/contentitems/externalcontent?o=ED"
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
